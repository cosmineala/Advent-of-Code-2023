...
const Day3Part1: React.FC<IProps> = () => {
...
function handleLine(matrix: ClassEngineMatrix, allLines: string[][], line: string[], y: number): number {
    ...
    for (let x = 0; x < line.length; x++) {
        ...
        if (isDigit && isReadingNumber === false) {
            lastNumber = {
                num: 0,
                begin: 0,
                end: 0,
        ...
        if (isDigit) {// let hasSpecial = false;
            degit: for (let i = -1; i <= 1; i++) {
                for (let j = -1; j <= 1; j++) {
                    ...
                    //     continue;
                    // }
                    if (lChar !== undefined && lChar !== "." && lChar !== "X" && !(lChar >= "0" && lChar <= "9")) {
        ...
        if (isReadingNumber && isDigit === false) {
    ...
    let sum = numbersCOrd.reduce((acc, curr) => acc + curr.num, 0);
    return sum;
